...
def get_parser():
    ...
    parser.add_argument("--exp_name", default='vqldm_2d_layerClassCond_seven_balanced')
    parser.add_argument('--result_root', type=str, default=r'/home/Data/xiebaoye/cldm_train_logs/ldm_train/logs')
    parser.add_argument('--data_config',
                        default=r'xxx/xxx/100edema_srf_seven.yaml')

    parser.add_argument('--first_stage_ckpt', type=str,
                        default='xxx/xxx/vqgan_pretrain/last.ckpt')
...
def main(opts, data_cfg):
    ...
    model = LDM(opts)
    if opts.command == "fit":
        wandb_logger = WandbLogger(project="layer2img", name=opts.exp_name, save_dir=opts.default_root_dir)
        # wandb_logger = None
        ...
        # ckpt = '/home/Data/xiebaoye/cldm_train_logs/ldm_train/logs/vqldm_2d_maskClassCond_total_balanced_2023-08-15T10-55-36/mask2img/83vx78j8/checkpoints/model-epoch=19.ckpt'
        # model.load_network(ckpt)
...
class LDM(DDPM_base):
    def __init__(self, opts):
        ...
        ddconfig = {'double_z': False, 'z_channels': 8, 'resolution': 256, 'in_channels': 1, 'out_ch': 1, 'ch': 32,
        ...
        unet_config = {'image_size': opts.latent_size, 'in_channels': opts.latent_channel + ddconfig['z_channels'],
                       ...
                       'num_heads': 8, 'use_scale_shift_norm': True, 'resblock_updown': True, 'num_classes': 7,
                       'ddconfig': ddconfig}
    ...
    def get_input(self, batch):
        ...
        c = [layer_cond, class_cond]
        encoder_posterior = self.encode_first_stage(x)
        z = self.scale_factor * self.get_first_stage_encoding(encoder_posterior).detach()
        return z, c, x

    def apply_model(self, x, t, cond):
        mask_cond, class_cond = cond
        return self.model(x=x, timesteps=t, mask_cond=mask_cond, class_cond=class_cond)
    ...
    def training_epoch_end(self, outputs):
        with self.ema_scope("Plotting"):
            ...
            mask_cond, class_cond = c[0].to('cpu'), c[1].to('cpu')
            x_rec = self.decode_first_stage(z).to('cpu')
            x = x.to('cpu')
            for i in range(x.shape[0]):
                save_name = str(i) + '_' + str(class_cond[i].item()) + '.png'
                denoise_img_row = []
                ...
                save_image([x[i] * 0.5 + 0.5, mask_cond[i], img_samples[i] * 0.5 + 0.5],
